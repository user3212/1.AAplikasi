...
# 1. PATCH Mapel1Screen.kt
file_path_mapel1 = 'app/src/main/java/com/example/ui/screens/Mapel1Screen.kt'
with open(file_path_mapel1, 'r') as f:
    content_mapel1 = f.read()

start_index = content_mapel1.find('@Composable\nfun Mapel1RombelView(')
if start_index == -1:
    print("Mapel1RombelView not found")
else:
    brace_count = 0
    in_function = False
    end_index = -1
    for i in range(start_index, len(content_mapel1)):
        if content_mapel1[i] == '{':
            brace_count += 1
            in_function = True
        elif content_mapel1[i] == '}':
            brace_count -= 1
            if in_function and brace_count == 0:
                end_index = i + 1
                break

    mapel1_code = """@Composable
fun Mapel1RombelView(
    ...
    new_content_mapel1 = content_mapel1[:start_index] + mapel1_code + content_mapel1[end_index:]
    with open(file_path_mapel1, 'w') as f:
        f.write(new_content_mapel1)
    print("Patched Mapel1Screen")
